...
typedef P1I<num_t, deltaFeeder<num_t, arctanFeeder<num_t, sumFeeder<num_t, idFeeder<num_t> > > > > patan_t;
...
#define int int64_t
  ...
  if(look < 0) {
    if(step < 0)
      qq = patan_t(max(abs(step) + int(3), (step - 1) * (step - 1)), abs(step), abs(look));
    else
      pp = shrinkMatrix<num_t, patan_t>(patan_t(stat * (step + 1), var, step + abs(look)), step);
  } else {
    if(step < 0)
      q = plin_t(max(abs(step) + int(3), (step - 1) * (step - 1)), abs(step), abs(look));
    else
      p = shrinkMatrix<num_t, plin_t>(plin_t(stat * (step + 1), var, step + abs(look)), step);
  }
  std::string s;
  num_t d(0);
  auto  Mx(d);
